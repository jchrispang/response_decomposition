...
% 1D hemodynamic decomposed response function as contours in a 
% x and time space to various stimuli
 
% Dependencies:
% 1. params.m from J.C. Pang
% 2. PoleDecomposition_num.m from J.C. Pang
% 3. DecomposedBOLD_with_wintegral1D.m from J.C. Pang

%%
function [Y, kz] = DecomposedBOLD_solution1D(p, v_b, Gamma, plotting, neuralDrive, vargin)
    ...
    switch neuralDrive
        ...
        case 'gaussian_block'
            ...
            stimulus = exp(-(sigmax^2/4)*kmat.^2).*(t_0*(sin(wmat*t_0/2)./(wmat*t_0/2))*exp(-1i*wmat*t_0/2));
        case 'gabor'
            switch length(vargin)
    ...
    [Y, kz] = DecomposedBOLD_with_wintegral1D(p, v_b, Gamma, stimulus);
    x = Y.position;
    t = Y.time;
    
%     figure()
%     contourf(x*1e3, t, real(Y.Ytotal))
%     set(gca, 'YDir', 'normal')
%     xlim([-5, 5])
%     ylim([0, 15])
    %% PLOTTING
    ...
    if plotting>0
        ...
        'fontweight', 'b', 'linestyle', 'none')
        title('Re[ Y_{3}(x, t) ]','fontsize',15);

        text(minSpace*1.65, maxTime*1.25, ...
        ...
        'fontweight', 'b', 'linestyle', 'none')
        title('Re[ Y_{total}(x, t) ]','fontsize',15);
        colorbar
        ...
        annotation('textbox', [0.635,0.34,0.1,0.1], 'String', '(f)', 'fontsize', 20, ...
        'fontweight', 'b', 'linestyle', 'none')
        title('Im[ Y_{total}(x, t) ]','fontsize',15);
        colorbar
        ...
        elseif plotting==2
        %%saving plot
            folder = ['Y1D_w=',num2str(freqMax),'_k=',num2str(spatialFreqMax),'/'];
            switch length(vargin)
                case 0
